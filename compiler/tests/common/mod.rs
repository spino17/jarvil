...
use compiler::ast::dump::dump_ast;
use compiler::build_ast;
use compiler::code::JarvilCode;
use compiler::codegen::python::PythonCodeGenerator;
use compiler::error::error::JarvilProgramAnalysisErrors;
use compiler::parser::resolver::JarvilResolver;
use compiler::parser::type_checker::JarvilTypeChecker;
...
// Transpiles to Python. Returns `Err` with the rendered diagnostics if any
// stage reported a problem, mirroring the CLI's refusal to emit code for a
// program that did not analyse cleanly.
pub fn transpile(source: &str) -> Result<String, String> {
    ...
    if !errors.is_empty() {
        return Err(render_diagnostics(&errors, source));
    }

    let py_generator = PythonCodeGenerator::new(&code_handler, semantic_db);

    Ok(py_generator.generate_python_code(&ast))
}

// Whether a usable `python3` is on PATH. The execution tests assert on real
// program output, so without an interpreter they can only be skipped -- a
// missing Python must not look like a compiler regression.
...
// Executes transpiled Python and returns its combined output. Panics on a
// non-zero exit, since generated code that crashes the interpreter is exactly
// the regression these tests exist to catch.
pub fn run_python(py_code: &str) -> String {
