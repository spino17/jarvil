# Compiler corpus tests

Snapshot tests over a corpus of `.jv` files, one directory per compiler stage.
Adding coverage means adding a `.jv` file — no Rust required.
...
corpus/
  ...
  codegen/          must compile cleanly; snapshot pins emitted Python *and* its output
```
...
- `codegen` tests need `python3` on `PATH`. Without it the execution half is
  skipped, not failed.
